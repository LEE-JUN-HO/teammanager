...
import {
  LayoutDashboard, Users, Settings, LogOut, ChevronRight,
  TrendingUp, Menu, X,
} from 'lucide-react'
...
export default function Sidebar() {
  const { currentUser, logout, selectedFiscalYear } = useAppStore()
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 pt-6 pb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-bold text-toss-gray-900 leading-tight">예산관리</p>
        ...
        <div className="bg-toss-gray-50 rounded-xl p-3">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-toss-gray-900 truncate">{currentUser?.name}</p>
              <p className="text-[11px] text-toss-gray-500 truncate">{currentUser?.email}</p>
            </div>
          </div>
          <button
  ...
  return (
    <>
      {/* Mobile toggle */}
      <button
        className="lg:hidden fixed top-4 left-4 z-50 w-10 h-10 bg-white rounded-xl shadow-card flex items-center justify-center"
      ...
      <aside
        className={clsx(
          'lg:hidden fixed left-0 top-0 bottom-0 z-50 w-64 bg-white shadow-modal transition-transform duration-300',
